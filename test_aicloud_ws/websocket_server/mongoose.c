 ...
 */
int cs_log_print_prefix(enum cs_log_level level, const char *fname, const char *func, int line);
...
#define LOG(l, x)                                     \
  do {                                                \
    if (cs_log_print_prefix(l, __FILE__, __func__, __LINE__)) { \
      cs_log_printf x;                                \
    }                                                 \
  } while (0)
...
int cs_log_print_prefix(enum cs_log_level level, const char *file, const char *func, int ln) WEAK;
int cs_log_print_prefix(enum cs_log_level level, const char *file, const char *func, int ln) {
  ...
  pl = fl + 1 + ll + strlen(func);
  memcpy(prefix, p, fl);
  snprintf(prefix + fl, sizeof(prefix) - fl, " %s", func); 
...
void mg_if_accept_tcp_cb(struct mg_connection *nc, union socket_address *sa,
  ...
  nc->sa = *sa;
#if MG_ENABLE_SSL
  LOG(LL_DEBUG, ("%p MG_F_SSL flag: %d", nc, nc->listener->flags));
  if (nc->listener->flags & MG_F_SSL) {
    nc->flags |= MG_F_SSL;
    LOG(LL_DEBUG, ("%p MG_F_SSL", nc));
